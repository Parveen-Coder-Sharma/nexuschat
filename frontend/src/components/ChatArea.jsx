...
import { 
  ...
  MdDarkMode, MdLightMode, MdKeyboardArrowDown, MdArrowBack, MdDownload 
} from 'react-icons/md';
    ...
    <>
      {fullscreenImage && (
        <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setFullscreenImage(null)}>
           <a href={fullscreenImage} target="_blank" rel="noopener noreferrer" download="NexusChat_Image.jpg" className="absolute top-6 right-20 text-white p-2 bg-white/10 rounded-full hover:bg-indigo-500 transition-colors z-50">
        ...
        <div className="flex items-center gap-2 sm:gap-4 overflow-hidden">
          ...
          <div className="overflow-hidden">
            ...
            <span className={`text-[10px] sm:text-xs font-medium truncate block max-w-[150px] sm:max-w-[250px] ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
              ...
              <>@{selectedUser.username || "username"} • {selectedUser.isOnline ? <span className="text-emerald-500 font-bold">Online</span> : (selectedUser.lastSeen ? formatLastSeen(selectedUser.lastSeen) : "Offline")}</>}
            </span>
          </div>
        </div>
        <div className="flex gap-1 sm:gap-2 text-slate-400 shrink-0">
      ...
      <div 
        ...
        className={`flex-1 overflow-y-auto overflow-x-hidden p-4 sm:p-6 pt-20 sm:pt-24 ${replyingTo ? 'pb-36' : 'pb-28'} space-y-4 sm:space-y-6 z-10 flex flex-col transition-all relative w-full`}
      >
        <div className={`self-center px-4 py-1 rounded-full text-[10px] sm:text-xs font-semibold shadow-sm border shrink-0 ${isDarkMode ? 'bg-slate-800 border-slate-700 text-slate-400' : 'bg-white border-slate-100 text-slate-400'}`}>{selectedUser.isGroup ? "Group Chat" : "Private Chat"}</div>
      ...
      <div className={`absolute bottom-0 w-full z-20 pb-4 sm:pb-6 pt-4 sm:pt-6 bg-gradient-to-t ${isDarkMode ? 'from-slate-900 to-transparent' : 'from-[#f8fafc] to-transparent'}`}>
        <div className="px-3 sm:px-6 relative w-full">
          {replyingTo && (
            <div className={`mb-2 mx-1 sm:mx-2 p-2 sm:p-3 rounded-t-2xl shadow-[0_-4px_10px_rgba(0,0,0,0.05)] border-t border-x flex justify-between items-start animate-in slide-in-from-bottom-2 ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
              <div className="flex flex-col border-l-4 border-indigo-500 pl-2 sm:pl-3 w-[85%]">
                ...
                <span className={`text-xs sm:text-sm truncate w-full ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>{replyingTo.documentUrl ? "📄 Document" : replyingTo.videoUrl ? "🎥 Video" : replyingTo.audioUrl ? "🎤 Voice Note" : (replyingTo.text || "📷 Photo")}</span>
              </div>
              <button onClick={() => setReplyingTo(null)} className={`p-1 rounded-full transition-colors outline-none shrink-0 ${isDarkMode ? 'text-slate-400 hover:text-red-400 hover:bg-slate-700' : 'text-slate-400 hover:text-red-500 hover:bg-slate-100'}`}><MdClose className="text-base sm:text-lg" /></button>
            </div>
          )}
          ...
          <div className={`p-1.5 sm:p-2 shadow-xl shadow-slate-200/50 border flex items-center gap-1 sm:gap-2 transition-all duration-300 w-full ${replyingTo ? 'rounded-b-2xl rounded-t-sm' : 'rounded-[2rem]'} ${isDarkMode ? 'bg-slate-800 border-slate-700 shadow-none' : 'bg-white border-slate-100'}`}>
            {isRecording ? (
              ...
              <div className="flex-1 flex items-center gap-1 sm:gap-2 animate-in fade-in overflow-hidden">
                <button onClick={() => setShowEmojiPicker(!showEmojiPicker)} className={`p-2 sm:p-3 rounded-full outline-none shrink-0 ${showEmojiPicker ? 'text-indigo-500 bg-indigo-500/10' : isDarkMode ? 'text-slate-400 hover:bg-slate-700' : 'text-slate-400 hover:bg-slate-50'}`}><MdMood className="text-xl sm:text-2xl" /></button>
                <button onClick={() => fileInputRef.current.click()} className={`p-2 sm:p-3 rounded-full outline-none shrink-0 ${isDarkMode ? 'text-slate-400 hover:bg-slate-700' : 'text-slate-400 hover:bg-slate-50'}`}><MdAttachFile className="text-xl sm:text-2xl" /></button>
                ...
                <input type="text" ref={inputRef} placeholder="Message..." value={message} onChange={handleInputChange} onKeyPress={(e) => { e.key === 'Enter' && handleSend() }} className={`flex-1 min-w-0 py-2 sm:py-3 px-3 sm:px-4 rounded-xl outline-none transition-all text-sm sm:text-base ${isDarkMode ? 'bg-slate-900/50 text-slate-200 placeholder-slate-500' : 'bg-slate-50/50 text-slate-700'}`} />
              </div>
            )}
            
            {isRecording ? <button onClick={stopRecording} className="bg-red-500 p-2.5 sm:p-3.5 rounded-full text-white shadow-lg shrink-0"><MdStop className="text-xl" /></button> 
            : message.trim() || replyingTo ? <button onClick={handleSend} className="bg-indigo-600 p-2.5 sm:p-3.5 rounded-full text-white shadow-lg shrink-0"><MdSend className="text-xl ml-1" /></button> 
            : <button onClick={startRecording} className="bg-emerald-500 p-2.5 sm:p-3.5 rounded-full text-white shadow-lg shrink-0"><MdMic className="text-xl" /></button>}
          </div>
        </div>
      </div>
