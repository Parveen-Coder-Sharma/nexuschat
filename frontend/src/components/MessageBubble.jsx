...
const MessageBubble = React.memo(({ msg, isMe, myId, selectedUser, handleReplyClick, setForwardMessageData, confirmDeleteMessage, handleScrollToMessage, formatTime, onReact, isDarkMode, onImageClick }) => {
  ...
  return (
    <div id={`msg-${msg._id}`} className={`flex flex-col gap-1 max-w-[75%] ${isMe ? 'self-end' : 'self-start'} group transition-all relative`}>
      ...
      <div className={`flex items-end gap-2 ${isMe ? 'flex-row' : 'flex-row-reverse'}`}>
        ...
        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity pb-2 relative">
        ...
        <div className={`message-bubble p-3 shadow-sm text-[15px] leading-relaxed break-words flex flex-col relative ${isMe ? 'bg-gradient-to-br from-indigo-600 to-indigo-500 rounded-2xl rounded-tr-sm text-white' : isDarkMode ? 'bg-slate-800 rounded-2xl rounded-tl-sm border border-slate-700 text-slate-200' : 'bg-white rounded-2xl rounded-tl-sm border border-slate-100 text-slate-700'}`}>
          ...
          {/* 🔥 FIX: Added onImageClick here for Fullscreen Viewer */}
          {msg.imageUrl && (
            <div className="mb-1.5 rounded-lg overflow-hidden bg-black/5 cursor-pointer" onClick={() => onImageClick(msg.imageUrl)}>
                <img src={msg.imageUrl} alt="Attachment" className="max-w-[220px] sm:max-w-[300px] max-h-64 object-contain rounded-lg hover:opacity-90 transition-opacity" />
            </div>
          )}
          
          {msg.videoUrl && (<div className="mb-1.5 rounded-lg overflow-hidden bg-black cursor-pointer"><video src={msg.videoUrl} controls preload="metadata" className="max-w-[220px] sm:max-w-[300px] max-h-64 object-contain rounded-lg" /></div>)}
          ...
          {msg.audioUrl && (<div className="mb-1.5"><audio src={msg.audioUrl} controls className={`h-10 w-[220px] sm:w-[260px] outline-none ${isMe ? 'opacity-90' : ''}`} /></div>)}
